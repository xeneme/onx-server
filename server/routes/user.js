    ...
    if (error) {
      ...
      User.findByIdAndUpdate(
        ...
        (err, user) => {
          ...
          Logger.register(user._id, 200, 'name_changed', 'action.user.nameChanged', user.firstName + ' ' + user.lastName)
          ...
          bcrypt.compare(req.body.password, user.password, (err, success) => {
            if (success) {
              ...
              User.findByIdAndUpdate(
                ...
                err => {
                  Logger.register(user._id, 200, 'password_changed', 'action.user.passwordChanged')
                  res.status(200).send({
                    token: UserToken.authorizationToken(userId),
